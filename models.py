...
class User(db.Model):
    ...
    collection_entries = db.relationship(
        "CollectionEntry", backref="user", lazy=True)
...
class Film(db.Model):
    ...
    collection_entries = db.relationship(
        "CollectionEntry", backref="film", lazy=True)
...
class CollectionEntry(db.Model):
    ...
    film_id = db.Column(db.String(36), db.ForeignKey(
    ...
    __table_args__ = (
        db.UniqueConstraint("user_id", "film_id",
